...
public class VerificationTokenServiceImpl extends BaseService implements VerificationTokenService {
    ...
    private VerificationToken sendEmailVerificationToken(User user) {
        VerificationToken token = new VerificationToken(user, VerificationTokenType.emailVerification, emailVerificationTokenExpiryTimeInMinutes);
    ...
    public VerificationToken sendEmailRegistrationToken(String userId) {
        User user = ensureUserIsLoaded(userId);
        VerificationToken token = new VerificationToken(user, VerificationTokenType.emailRegistration, emailRegistrationTokenExpiryTimeInMinutes);
        tokenRepository.save(token);
    ...
    public VerificationToken sendLostPasswordToken(LostPasswordRequest lostPasswordRequest) {
        ...
        if (user != null) {
            ...
            if (token == null) {
                token = new VerificationToken(user, VerificationTokenType.lostPassword, lostPasswordTokenExpiryTimeInMinutes);
                tokenRepository.save(token);
            }
